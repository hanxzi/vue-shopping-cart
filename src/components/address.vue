    ...
    <div class="container">
      <div class="checkout-addr">
        ...
        <div class="addr-list-wrap">
          <div class="addr-list">
            <ul>
              <li v-for="(item,index) in filteredItems" v-bind:class="{'check':index==curindex}" v-on:click="curindex=index">
                <dl>
                  <dt>{{item.userName}}</dt>
                ...
                <div class="addr-opration addr-del">
                  <a href="javascript:;" class="addr-del-btn"  v-on:click="deleteConfirm(item)">
                    <svg class="icon icon-del"><use xlink:href="#icon-del"></use></svg>
                  </a>
                </div>
                <div class="addr-opration addr-set-default">
                  <a href="javascript:;" class="addr-set-default-btn" v-on:click="setDefault(item.addressId)" v-if="!item.isDefault"><i>设为默认</i></a>
                </div>
          ...
          <div class="shipping-addr-more">
            <a class="addr-more-btn up-down-btn" href="javascript:" v-on:click="limitNum=addressList.length">
              more
              <i class="i-up-down">
        ...
        <div class="shipping-method-wrap">
          <div class="shipping-method">
            <ul>
              <li v-bind:class="{'check':shippingMethod==1}" v-on:click="shippingMethod=1">
                <div class="name">标准配送</div>
                <div class="price">Free</div>
              </li>
              <li v-bind:class="{'check':shippingMethod==2}" v-on:click="shippingMethod=2">
        ...
        <div class="md-modal modal-msg md-modal-transition" id="showModal" v-bind:class="{'md-show':isDelete}">
          <div class="md-modal-inner">
            <div class="md-top">
              <button class="md-close" v-on:click="isDelete = false">关闭</button>
            </div>
            <div class="md-content">
              ...
              <div class="btn-wrap col-2">
                <button class="btn btn--m" id="btnModalConfirm" v-on:click="delAddress()" >Yes</button>
                <button class="btn btn--m btn--red" id="btnModalCancel" v-on:click="isDelete = false">No</button>
              </div>
            </div>
          </div>
        </div>
        <div class="md-modal modal-msg md-modal-transition" id="showModalw">
         ...
         <div class="md-overlay" id="showOverLay" v-if="isDelete"></div>
      </div>
    </div>
  </div>
</template>
...
export default {
  ...
  methods:{
    ...
    delAddress(){
      var _index = this.addressList.indexOf(this.curAddress);
